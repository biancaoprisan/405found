...
class Project extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(`http://localhost:3003/api/users/${user.id_user}/projects`)
      .then(response => {
  ...
  handleNewProject = () => {
    axios
      .post('http://localhost:3003/api/project', {name: this.state.inputProjectName, link: this.state.inputProjectLink})
      .then(response => {
        this.handleClose();
      }).catch(error => {
        this.handleClose();
